...
function setSessionLang(elem){
	var lang = $(elem).val();
	$.ajax({
	  type: 'POST',
	  url: ajaxSetSessionLangUrl,
	  data: {'lang':lang},
	  beforeSend: showPageLoading,
	  success: function(data) {
	  	location.reload();
...
function modernaChangeVcDate(elem){
	...
	if(data.order_lot_system.indexOf('moderna')!=-1){
		openModal("กรุณากดยืนยัน",'<b style="color:red; font-size:20px;">ท่านสามารถเลื่อนนัดได้ตลอดก่อนถึงวันฉีดจริง 24 ชม.</b><br/><br/><img src="https://cdn.jsdelivr.net/gh/pop-itsmartone/service-cdn@main/image/คำแนะนำ_moderna_4.jpg" style="max-width:100%" />',true,function(){
			hideModal();
...
function modernaChangeReceiver(elem){
	...
	if(data.order_lot_system.indexOf('moderna')!=-1){
		//openModal("กรุณากดยืนยัน",'<b style="color:red; font-size:20px;">ท่านสามารถโอนสิทธิ์ได้เพียง 1 ครั้งเท่านั้น</b><br/><br/><img src="https://cdn.jsdelivr.net/gh/pop-itsmartone/service-cdn@main/image/คำแนะนำ_moderna_4.jpg" style="max-width:100%" />',true,function(){
		openModal("กรุณากดยืนยัน",'<img src="https://cdn.jsdelivr.net/gh/pop-itsmartone/service-cdn@main/image/คำแนะนำ_moderna_4.jpg" style="max-width:100%" />',true,function(){
			hideModal();
	            ...
	            { 	"mData" : null, "sClass": "center", "bVisible": btnConfirmVisible,
					"mRender": function ( data, type, full ) {
						...
						if(full.has_confirm_appt=='F'){
							// html+='รอยืนยัน';
							html+=_langs['wait_confirm'];
							if(full.vc_date_time_text!=null && full.vc_date_time_text!=''){
								html+='<br/><a href="#" class="btn btn-success btn-sm" style="width:90px;" onclick="modernaConfirmAppt(\''+full.vaccine_moderna_id+'\',\'T\'); return false;">'+_langs['click_confirm']+'</a>';
							}
						}
						else{
	            ...
	            { 	"mData" : null, "sClass": "center",
					"mRender": function ( data, type, full ) {
						...
						if(full.can_transfer=='T'){
							html+=full.status_text+'<br/><a href="#" style="width:90px;" class="btn btn-primary btn-sm" onclick="modernaChangeReceiver(this); return false;">'+_langs['transfer_vaccine_rights']+'</a>';
						}
						else{
					...
					"mRender": function ( data, type, full ) {
						...
						*/
						...
						if(full.can_change_date=='T'){
							if(full.appt_vaccine_date==null){
								html+=nl2br(full.vc_date_time_text)+'<br/><a href="#" style="width:100px;" class="btn btn-primary btn-sm" onclick="modernaChangeVcDate(this);">'+_langs['click_assign_date']+'</a>';
							}
							else{
								html+=nl2br(full.vc_date_time_text)+'<br/><a href="#" style="width:100px;" class="btn btn-primary btn-sm" onclick="modernaChangeVcDate(this);">'+_langs['click_change_date']+'</a>';
							}
						}
						else{
...
function checkCovidSendOtp(){
	...
	if(passValidate){
		...
		$.ajax({
		  ...
		  data: {"otp_type":otpType,"phone":phone,"email":email,"pid":pid,"hos_code":hos_code},
		  beforeSend: showPageLoading,
		  success: function(data) {
		  	if(data.error=='Not Found'){
		  		$("#div-inquiry").show();
  	...
  	// info.url = URL.createObjectURL(b64toBlob(info.base64, 'application/pdf'));
		...
		*/
		if(info.doc_type=='med_cert' || $.inArray(lab_result_l, ["detect", "detected", "undetected", "undetectable"
				, "see scan", "equivocal", "to follow", "Wait for result", "positive", "negative" ]) != -1){
	    var detected = (info.lab_result.toLowerCase()=='detect' || info.lab_result.toLowerCase()=='detected' || info.lab_result.toLowerCase()=='positive' || info.lab_result.toLowerCase()=='equivocal' || (info.resultName=='COVID19 Real time PCR' && info.lab_result=='SEE SCAN'));
			var hos_code = info.hos_code;
			if(hos_code=='500' && detected){
				html+='<b style="color:#000; display:block;">ไม่พบข้อมูล / Not Found</b><br/>';
				html+='<b style="color:red; display:block;">กรุณาติดต่อเจ้าหน้าที่ <a href="https://bpkconnect.com">https://bpkconnect.com</a> ขออภัยในความไม่สะดวก</b><br/>';
			}
			else{
...
function vaccineMonitorChangeApptDate(elem){
	...
	$('#inputCDVcDate',form).change(function(){
		$('#changeVcDateModal #button-save').show();
		if($('#inputCDVcDate option:selected',form).hasClass('day-off')){
			if(userBranchcode=='EXT'){
				alert('ไม่สามารถนัดหมายวันนี้ได้');
				$('#changeVcDateModal #button-save').hide();
			}
			else{
	...
	$(function(){
		bpkDataTable=$('#vaccine_monitor-datatable').DataTable({
			...
			"aoColumns": [
	            ...
	            { 	"mData" : null, "sClass": "center",
					"mRender": function ( data, type, full ) {
						var html = nl2br(full.product_detail);
						if(full.order_from=='offline'){
							html+=' (offline)';
						}
						return html;
					}
	            ...
	            { 	"mData" : null, "sClass": "center",
			      ...
			      if(full.is_paid=='T' && $.inArray(full.oc_product_id,['82','83','96'])!=-1){
            	...
            	if(html=='') return '';
		            ...
		            return returnHtml;
		            		...
		            		if((full.vaccine_status_id=='W' && inJSONArray("CHANGE_VACCINE_RECEIVER",session_service_permission_include) != -1)
		            			|| userEmpId=='2163' || userEmpId=='system' || userEmpId=='56000048'){
...
//order_vaccine-index
function orderVaccinePrintReceipt(order_id){
	window.open(printReceiptVaccinePdf3Url+"/"+order_id);
}
function orderVaccinePrintBooking(order_id){
	window.open(printBookingVaccineUrl+"/"+order_id);
}
function orderVaccineCancel(order_id){
	openModal("ยืนยัน","ยืนยันยกเลิกรายการ?",true,function(){
		$.ajax({
			type: 'POST',
			url: orderVaccineCancelUrl,
...
function orderVaccineReceivePaymentConfirm(){
	var form = $("#receivePaymentModalForm");
	var passValidate=true;
	//check input bottom up
	passValidate=checkInputTypeSelect($("select[name=inputRVPaymentMethodId]",form)) && passValidate;
	...
	if(passValidate){
		$.ajax({
		  type: 'POST',
		  url: orderVaccineReceivePaymentUrl,
		  data:  form.serialize(),
		  beforeSend: showPageLoading,
		  success: function(data) {
		  	if(data.error!=''){
		  	...
		  	$('#receivePaymentModal').modal('hide');
				openModal("success","บันทึกสำเร็จ",false,null);
				bpkDataTableSearchSubmit(bpkDataTable,false);
				orderVaccinePrintReceipt($('#inputReceivePaymentOrderId',form).val());
			},
		  dataType: "json",
		  async:true,
		  complete: hidePageLoading
...
function orderVaccineReceivePayment(elem){
	var tableElem = $(elem).closest('table.dataTable');
	var table = $(tableElem).DataTable();
	var trElem = $(elem).closest('tr');
	if(trElem.hasClass('child')){
	...
	$("#inputReceivePaymentOrderId",form).val(rowData.order_id);
	$("#inputRVOrderId",form).val(rowData.order_id);
	$("#inputRVFullname",form).val(rowData.fullname);
	$("#inputRVPid",form).val(rowData.pid);
	$("#inputRVPaymentMethodId",form).val('');
	$("#inputRVQuantity",form).val(rowData.quantity);
	$("#inputRVTotalAmt",form).val(rowData.total_amt);
	$("#inputRVDiscount",form).val(rowData.discount);
	$("#inputRVNetAmt",form).val(rowData.net_amt);
...
function orderVaccineAddOrderConfirm(){
  var form=$("#addOrderModalForm");
	...
	var passValidate=true;
	//check input bottom up
	$(".form-group.required input[type=text]:visible",form).each(function(){
		updateInputErrMsg('',$(this));
		passValidate=checkInputTypeText($(this)) && passValidate;
	})
	$(".form-group.required select:visible",form).each(function(){
		updateInputErrMsg('',$(this));
		passValidate=checkInputTypeSelect($(this)) && passValidate;
	})

	updateInputErrMsg('',$("#inputAddOrderDiscount",form));
	updateInputErrMsg('',$("#inputAddOrderDiscountAttachment",form));

	if($("#inputAddOrderDiscount",form).val()!=''){
		var discount = parseFloat($("#inputAddOrderDiscount",form).val());
		var qty = parseFloat($("#inputAddOrderQuantity",form).val());
		if(discount/qty > 490){
			updateInputErrMsg('ส่วนลดต่อเข็มห้ามเกิน 490 บาท',$("#inputAddOrderDiscount",form));
			passValidate = false;
		}
		else{
		}
		...
		if(discount>0){
			var foundFile = false;
			var fileCount = $('input[type=file][name^=inputAddOrderDiscountAttachment]',form).size();
			for(var i=0;i<fileCount;i++){
				if($('input[type=file][name^=inputAddOrderDiscountAttachment]',form).get(i).files.length>0){
					foundFile=true;
					break;
				}
			}
			if(!foundFile){
				passValidate=false;
				updateInputErrMsg('กรุณาแนบไฟล์',$("#inputAddOrderDiscountAttachment",form));
			}
	...
	if(passValidate){
		$.ajax({
			type: 'POST',
			url: orderVaccineAddOrderSubmitUrl,
    	...
    	contentType: false,
	      ...
	      if(data.error!=""){
	        ...
	        openModal("success","บันทึกสำเร็จ",false,null);
	        bpkDataTableSearchSubmit();
	        if(viewType!='order_ext'){
						orderVaccinePrintBooking(data.order_id);
					}
	      }
			}
		});
...
function orderVaccineAddOrderCalTotal(){
	var form = $("#addOrderModalForm");
	var price = $("#inputAddOrderVaccineTypeId option:selected",form).attr('price');
	$("#inputAddOrderVaccinePrice",form).val(price);
	var quantity = $("#inputAddOrderQuantity",form).val();
	var totalAmt = parseFloat(price)*parseFloat(quantity);
	var discount = $("#inputAddOrderDiscount",form).val();
	if(discount=='') discount = 0;
	var netAmt = totalAmt - parseFloat(discount);
	$("#inputAddOrderTotalAmt",form).val(parseFloat(totalAmt).formatMoney(2,'.',','));
	$("#inputAddOrderNetAmt",form).val(parseFloat(netAmt).formatMoney(2,'.',','));
}
function orderVaccineAddOrderModalInitNationality(){
	$("#inputAddOrderNationality",form).val('THA');
	$("#inputAddOrderPid",form).closest('.form-group').parent().show();
	$("#inputAddOrderPid",form).closest('.form-group').addClass('required');
	$("#inputAddOrderPassport",form).closest('.form-group').parent().hide();
	$("#inputAddOrderPassport",form).closest('.form-group').removeClass('required');
}
function orderVaccineAddOrderGetMonthList(){
	var form = $("#addOrderModalForm");
	var vaccine_place_id = $('#inputAddOrderVaccinePlaceId',form).val();
	var vaccine_type_id = $('#inputAddOrderVaccineTypeId',form).val();
	$('#inputAddOrderVcMonth',form).html('');

	$.ajax({
	  type: 'POST',
	  url: orderVaccineGetVcMonthListUrl,
	  data: {"vaccine_place_id":vaccine_place_id,"vaccine_type_id":vaccine_type_id},
	  beforeSend: showPageLoading,
	  success: function(data) {
	  	if(data.error!=''){
				openModal("fail",data.error,false,null);
	  	...
	  	$('#inputAddOrderVcMonth',form).html(data.html);
		},
	  dataType: "json",
	  async:true,
	  complete: hidePageLoading
...
function orderVaccineAddOrderGetDateList(){
	var form = $("#addOrderModalForm");
	var vaccine_place_id = $('#inputAddOrderVaccinePlaceId',form).val();
	var vaccine_type_id = $('#inputAddOrderVaccineTypeId',form).val();
	$('#inputAddOrderVcDate',form).html('');

	var vc_month = $('#inputAddOrderVcMonth',form).val();
	$.ajax({
	  type: 'POST',
	  url: orderVaccineGetVcDateListUrl,
	  data: {"vaccine_place_id":vaccine_place_id,"vaccine_type_id":vaccine_type_id,"vc_month":vc_month},
	  beforeSend: showPageLoading,
	  success: function(data) {
	  	if(data.error!=''){
				openModal("fail",data.error,false,null);
	  	...
	  	$('#inputAddOrderVcDate',form).html(data.html);
		},
	  dataType: "json",
	  async:true,
	  complete: hidePageLoading
...
function orderVaccineAddOrderGetTimeList(){
	var form = $("#addOrderModalForm");
	var vaccine_place_id = $('#inputAddOrderVaccinePlaceId',form).val();
	var vaccine_type_id = $('#inputAddOrderVaccineTypeId',form).val();
	var vc_date = $('#inputAddOrderVcDate',form).val();
	$('#inputAddOrderVcTime',form).html('');

	$.ajax({
	  type: 'POST',
	  url: orderVaccineGetVcTimeListUrl,
	  data: {"vaccine_place_id":vaccine_place_id,"vaccine_type_id":vaccine_type_id,"vc_date":vc_date},
	  beforeSend: showPageLoading,
	  success: function(data) {
	  	if(data.error!=''){
				openModal("fail",data.error,false,null);
	  	...
	  	$('#inputAddOrderVcTime',form).html(data.html);
		},
	  dataType: "json",
	  async:true,
	  complete: hidePageLoading
...
function openOrderVaccineAddOrderModal(){
	var form = $("#addOrderModalForm");
	$("input[type=text]",form).val('');
	$("input[type=file]",form).val('');
	$("#inputAddOrderIsExt",form).val('F');
	$("#inputAddOrderOrderId",form).val('');
	$("#inputAddOrderQuantity",form).val('1');
	$("#inputAddOrderIsVip",form).prop('checked',false);
	orderVaccineAddOrderCalTotal();

	if(viewType=='order_ext'){
		$("#inputAddOrderCustomerType",form).val('vc-company');
		$("#inputAddOrderTaxBranchcode",form).val('00000');
		$("#inputAddOrderIsExt",form).val('T');
		$("#inputAddOrderIsPaid",form).val('T');
		// $("#inputAddOrderDiscount",form).prop('readonly',false);
	}
	else{
		$("#inputAddOrderCustomerType",form).val('vc-person');
		$("#inputAddOrderEmail",form).closest('.form-group').removeClass('required');
		$("#inputAddOrderIsPaid",form).val('F');
		// $("#inputAddOrderDiscount",form).prop('readonly',true);
		$("div.vc-person",form).show();
		$("div.vc-company",form).hide();
		$('#inputAddOrderVcDate',form).html('');
		$('#inputAddOrderVcTime',form).html('');
		orderVaccineAddOrderGetMonthList();
		orderVaccineAddOrderModalInitNationality();
	}

	$('#addOrderModal').modal({ dynamic: true, backdrop: false });
	setTimeout(function(){
		$('#addOrderModal').focus(); // add to fix bug on chrome when press esc
	},300);
}
if(viewName=="order_vaccine-index"){
	var form = $("#addOrderModalForm");
	
	$('#inputAddOrderVcMonth',form).change(function(){
		orderVaccineAddOrderGetDateList();
	});
	$('#inputAddOrderVcDate',form).change(function(){
		orderVaccineAddOrderGetTimeList();
		if($('#inputAddOrderVcDate option:selected',form).hasClass('day-off')){
			alert('วันนี้ไม่ได้เปิด แต่สามารถทำนัดได้ถ้าเป็นเคสที่อนุมัติแล้ว');
		}
		else if($('#inputAddOrderVcDate option:selected',form).hasClass('day-full')){
			alert('วันนี้เต็มแล้ว แต่สามารถทำนัดได้ถ้าเป็นเคสที่อนุมัติแล้ว');
		}
	});

	$("#inputAddOrderCustomerType",form).change(function(){
		if($(this).val()=='vc-company'){
			$("div.vc-company",form).show();
			$("div.vc-person",form).hide();
			// $("#inputAddOrderDiscount",form).prop('readonly',false);
			$("#inputAddOrderEmail",form).closest('.form-group').addClass('required');
		}
		else{
			$("div.vc-person",form).show();
			$("div.vc-company",form).hide();
	...
	$("#inputAddOrderDiscount",form).keyup(function(){
		var discount = $(this).val();
		if($.trim(discount)!=''){
			$("#inputAddOrderDiscountReason",form).closest('.form-group').addClass('required');
			$("#inputAddOrderDiscountAttachment",form).closest('.form-group').addClass('required');
		}
		else{
			$("#inputAddOrderDiscountReason",form).closest('.form-group').removeClass('required');
			$("#inputAddOrderDiscountAttachment",form).closest('.form-group').removeClass('required');
		}
	});

	$("#inputAddOrderNationality",form).change(function(){
		var form = $("#addOrderModalForm");
		if($(this).val()=='THA'){
			$("#inputAddOrderPid",form).closest('.form-group').parent().show();
			$("#inputAddOrderPid",form).closest('.form-group').addClass('required');
			...
			$("#inputAddOrderPid",form).closest('.form-group').parent().hide();
			$("#inputAddOrderPid",form).closest('.form-group').removeClass('required');
			$("#inputAddOrderPassport",form).closest('.form-group').parent().show();
			$("#inputAddOrderPassport",form).closest('.form-group').addClass('required');
		}
	});
	
	$("#inputAddOrderVaccineTypeId",form).change(function(){
		orderVaccineAddOrderCalTotal();
	});

	$("#inputAddOrderQuantity, #inputAddOrderDiscount",form).keyup(function(){
		orderVaccineAddOrderCalTotal();
	});
	...
	$(function(){
		bpkDataTable=$('#report-datatable').DataTable({
			...
			"aoColumns": [
	            ...
	            { 	"mData" : null, "sClass": "center",
					"mRender": function ( data, type, full ) {
						var html = nl2br(full.product_detail);
						if(full.discount_reason!=null){
							html+='<br/>(เหตุผลที่ลด: '+full.discount_reason+')';
						}
						if(full.discount_attachment_path!=null){
							html+='<br/><a href="'+base_url+full.discount_attachment_path+'" target="_blank">'+full.discount_attachment_name+'</a>';
						}
						return html;
					}
				},
          ...
          "mRender": function ( data, type, full ) {
            return full.quantity;
	            ...
	            { 	"mData" : null, "sClass": "right",
          "mRender": function ( data, type, full ) {
          	if(full.net_amt==null || full.net_amt=='') return '';
            return parseFloat(full.net_amt).formatMoney(2,'.',',');
          }
				},
	            { 	"mData" : null, "sClass": "center",
					"mRender": function ( data, type, full ) {
	            ...
	            { 	"mData" : null, "sClass": "center",
					"mRender": function ( data, type, full ) {
						return full.payment_datetime;
					}
				},
	            { 	"mData" : null, "sClass": "center",
					"mRender": function ( data, type, full ) {
						var html = '';
            if(full.order_status_id=="5"){
              html+='<span class="label label-success" style="font-size:12px;">'+full.order_status_detail+'</span>';
            }
            else if(full.order_status_id=="7"){
              html+='<span class="label label-danger" style="font-size:12px;">'+full.order_status_detail+'</span>';
            }
            else{
              html+='<span class="label label-warning" style="font-size:12px;">'+full.order_status_detail+'</span>';
            }
				...
				},
        {
        	"mData" : null, "sClass": "center",
					"mRender": function ( data, type, full ) {
						var returnHtml = '';
		        if(full.order_status_id=="0"){
      				returnHtml+='<li><a href="#" onclick="orderVaccineCancel(\''+full.order_id+'\'); return false;"><span>ยกเลิกรายการ</span></a></li>';
		        }
		        else if(full.order_status_id=="1" && inJSONArray("ORDER_VACCINE_CASHIER",session_service_permission_include) != -1){
      				returnHtml+='<li><a href="#" onclick="orderVaccineReceivePayment(this); return false;"><span>รับเงินค่าวัคซีน</span></a></li>';
      				returnHtml+='<li><a href="#" onclick="orderVaccineCancel(\''+full.order_id+'\'); return false;"><span>ยกเลิกรายการ</span></a></li>';
		        }
		        else{
			        if(full.order_status_id=="1" && full.is_ext =='F' && inJSONArray("ORDER_VACCINE",session_service_permission_include) != -1){
	      				returnHtml+='<li><a href="#" onclick="orderVaccineCancel(\''+full.order_id+'\'); return false;"><span>ยกเลิกรายการ</span></a></li>';
			        }
			        else if(full.order_status_id=="1" && full.is_ext =='T' && inJSONArray("ORDER_VACCINE_EXT",session_service_permission_include) != -1){
	      				returnHtml+='<li><a href="#" onclick="orderVaccineCancel(\''+full.order_id+'\'); return false;"><span>ยกเลิกรายการ</span></a></li>';
			        }
		        }

		        if(full.order_status_id=="1" && full.is_ext=='F'){
      				returnHtml+='<li><a href="#" onclick="orderVaccinePrintBooking(\''+full.order_id+'\'); return false;"><span>พิมพ์ใบจอง</span></a></li>';
		        }
		        if(full.order_status_id=="5"){
      				returnHtml+='<li><a href="#" onclick="orderVaccinePrintReceipt(\''+full.order_id+'\'); return false;"><span>พิมพ์ใบเสร็จ</span></a></li>';
		        }
...
//report_ecommerce-customer_purchased_by_doctor
function validateReportEcommerceCustomerPurchasedByDoctorForm(){
  var passValidate=true;
  //check input bottom up
  // passValidate=checkInputTypeDateBetween($("#inputDateStart"),$("#inputDateEnd")) && passValidate;
  return passValidate;
}
if(viewName=="report_ecommerce-customer_purchased_by_doctor"){
  var bpkDataTable=$('#report-datatable').DataTable({
    "bFilter": true,
    "responsive": true,
    "searchDelay": sSearchDelay,
    ...
    "aoColumns": [
      ...
      {   "mData" : null, "sClass": "center",
        "mRender": function ( data, type, full ) {
          return returnEmptyIfNull(full.telephone);
        }
      },
      {   "mData" : null, "sClass": "center",
        "mRender": function ( data, type, full ) {
      ...
      {   "mData" : null, "sClass": "left",
        "mRender": function ( data, type, full ) {
          return returnEmptyIfNull(full.address);
        }
      },
      {   "mData" : null, "sClass": "center",
        "mRender": function ( data, type, full ) {
          return returnEmptyIfNull(full.doctor_username);
        }
      },
      {   "mData" : null, "sClass": "left",
        "mRender": function ( data, type, full ) {
          return returnEmptyIfNull(full.doctor_name);
        }
      },
      {   "mData" : null, "sClass": "right",
        "mRender": function ( data, type, full ) {
...
//end report_ecommerce-customer_purchased_by_doctor


//report_ecommerce-product_purchased
function validateReportEcommerceProductPurchasedForm(){
  var passValidate=true;
  //check input bottom up
  // passValidate=checkInputTypeDateBetween($("#inputDateStart"),$("#inputDateEnd")) && passValidate;
  return passValidate;
}
if(viewName=="report_ecommerce-product_purchased"){
  var bpkDataTable=$('#report-datatable').DataTable({
    "bFilter": true,
    "responsive": true,
    "searchDelay": sSearchDelay,
    ...
    "aoColumns": [
      ...
      {   "mData" : null, "sClass": "center",
        "mRender": function ( data, type, full ) {
          return returnEmptyIfNull(full.receipt_no);
        }
      },
      {   "mData" : null, "sClass": "right",
        "mRender": function ( data, type, full ) {
          return parseFloat(full.quantity).formatMoney(2,'.',',');
        }
      },
      {   "mData" : null, "sClass": "right",
        "mRender": function ( data, type, full ) {
      ...
      {   "mData" : null, "sClass": "right",
        "mRender": function ( data, type, full ) {
          return parseFloat(full.line_total).formatMoney(2,'.',',');
        }
      },
      {   "mData" : null, "sClass": "left",
        "mRender": function ( data, type, full ) {
      ...
      {   "mData" : null, "sClass": "left",
        "mRender": function ( data, type, full ) {
          return returnEmptyIfNull(full.sale_name);
        }
      },
      {   "mData" : null, "sClass": "right",
        "mRender": function ( data, type, full ) {
          return parseFloat(full.doctor_commission).formatMoney(2,'.',',');
        }
      },
      {   "mData" : null, "sClass": "right",
        "mRender": function ( data, type, full ) {
          return parseFloat(full.sale_commission).formatMoney(2,'.',',');
        }
      }
    ],
    "aaSorting": [[ 1, "asc" ]],
...
//end report_ecommerce-product_purchased


//report_ecommerce-doctor_commission
function validateReportEcommerceDoctorCommissionForm(){
  var passValidate=true;
  //check input bottom up
  // passValidate=checkInputTypeDateBetween($("#inputDateStart"),$("#inputDateEnd")) && passValidate;
  return passValidate;
}
if(viewName=="report_ecommerce-doctor_commission"){
  var bpkDataTable=$('#report-datatable').DataTable({
    "bFilter": true,
    "responsive": true,
    "searchDelay": sSearchDelay,
...
//end report_ecommerce-doctor_commission


//report_ecommerce-sale_commission
function validateReportEcommerceSaleCommissionForm(){
  var passValidate=true;
  //check input bottom up
  // passValidate=checkInputTypeDateBetween($("#inputDateStart"),$("#inputDateEnd")) && passValidate;
  return passValidate;
}
if(viewName=="report_ecommerce-sale_commission"){
  var bpkDataTable=$('#report-datatable').DataTable({
    "bFilter": true,
    "responsive": true,
    "searchDelay": sSearchDelay,
...
//end report_ecommerce-sale_commission
...
function modernaSendEmailVaccineCompanyUser(order_id){
	openModal("ยืนยัน","ยืนยันส่งเมล์หรือไม่?",true,function(){
		$.ajax({
		  type: 'POST',
		  url: apiVaccineSendEmailCompanyUserUrl,
		  data:  {'order_id':order_id,'emp_id':userEmpId},
		  beforeSend: showPageLoading,
		  success: function(data) {
		  	if(data.error!=''){
					openModal("fail",data.error,false,null);
...
function modernaGenVaccineCompanyUser(order_id){
	openModal("ยืนยัน","ยืนยัน Gen User หรือไม่?",true,function(){
		$.ajax({
		  type: 'POST',
		  url: apiVaccineGenCompanyUserUrl,
		  data:  {'order_id':order_id,'emp_id':userEmpId},
		  beforeSend: showPageLoading,
		  success: function(data) {
		  	if(data.error!=''){
...
function modernaEditEmailVaccineCompany(order_id,elem){
	$("#editEmailModal .modal-title").html('แก้ไข Email');
	...
	var form = $("#editEmailModalForm");
	$("#inputEditEmailModalForm_OrderId",form).val(order_id);
	$("#inputEditEmailModalForm_Email",form).val(data.email);

	$('#editEmailModal').modal({ dynamic: true, backdrop: false });
	setTimeout(function(){
		$('#editEmailModal').focus(); // add to fix bug on chrome when press esc
	},300);
}
function modernaEditEmailVaccineCompanyConfirm(){
	var form = $("#editEmailModalForm");

  var passValidate=true;
  //check input bottom up
  passValidate=checkInputTypeText($("#inputEditEmailModalForm_Email",form)) && passValidate;
  
  if(passValidate){
		var order_id=$("#inputEditEmailModalForm_OrderId",form).val();
		var email=$("#inputEditEmailModalForm_Email",form).val();

		$.ajax({
		  type: 'POST',
		  url: apiVaccineEditEmailCompanyUrl,
  ...
  var bpkDataTable=$('#report-datatable').DataTable({
    ...
    "aoColumns": [
      ...
      {   "mData" : null, "sClass": "center",
        "mRender": function ( data, type, full ) {
      		var html = full.email;
          if(inJSONArray("EDIT_VACCINE_COMPANY_EMAIL",session_service_permission_include) != -1){
						html+='<br/><a href="#" onclick="modernaEditEmailVaccineCompany(\''+full.oc_order_id+'\',this); return false;">แก้ไข Email</a>';
      		}
      ...
      {   "mData" : null, "sClass": "center",
        "mRender": function ( data, type, full ) {
      		return full.hos_queue_no_text;
        }
      },
      {   "mData" : null, "sClass": "center",
        "mRender": function ( data, type, full ) {
        	if(full.order_lot_system=='moderna-2'){
        		var html = '';
	        	if(full.has_account=='F'){
              html+='<span>รอ Gen</span>';
		          if(inJSONArray("GEN_VACCINE_COMPANY_USER",session_service_permission_include) != -1){
								html+='<br/><a href="#" class="btn btn-primary btn-sm" style="width:120px;" onclick="modernaGenVaccineCompanyUser(\''+full.oc_order_id+'\'); return false;">Gen User และส่งเมล์</a>';
	        		}
	        	}
	        	else{
              html+='<span>Gen แล้ว</span>';
		          if(inJSONArray("GEN_VACCINE_COMPANY_USER",session_service_permission_include) != -1){
								html+='<br/><a href="#" class="btn btn-success btn-sm" style="width:120px;" onclick="modernaSendEmailVaccineCompanyUser(\''+full.oc_order_id+'\'); return false;">ส่งเมล์อีกครั้ง</a>';
	        		}
	        	}
	        	return html;
        	}
...
//report-order_vaccine
function validateReportOrderVaccineForm(){
  var passValidate=true;
  //check input bottom up
  //passValidate=checkInputTypeDateBetween($("#inputDateStart"),$("#inputDateEnd")) && passValidate;
  return passValidate;
}
if(viewName=="report-order_vaccine"){
		bpkDataTable=$('#report-datatable').DataTable({
			"responsive": true,
			"searchDelay": sSearchDelay,
			"sPaginationType": "full_numbers",
...
//end report-order_vaccine


//report-order_test_and_go
function validateReportOrderTestAndGoForm(){
  var passValidate=true;
  //check input bottom up
  //passValidate=checkInputTypeDateBetween($("#inputDateStart"),$("#inputDateEnd")) && passValidate;
  return passValidate;
}
if(viewName=="report-order_test_and_go"){
    bpkDataTable=$('#report-datatable').DataTable({
      "responsive": true,
      "searchDelay": sSearchDelay,
      "sPaginationType": "full_numbers",
      ...
      "aoColumns": [
        ...
        "mRender": function ( data, type, full ) {
          ...
          "mRender": function ( data, type, full ) {
            var html = nl2br(full.product_detail);
            if(full.order_attachment_paths!=null){
            	for(var i in full.order_attachment_paths){
            		var order_attachment_path = full.order_attachment_paths[i];
             		html+='<br/><a href="https://bpkconnect.com/ecommerce/'+order_attachment_path+'" target="_blank">'+full.order_attachment_names[i]+'</a>';
            	}
            }
            return html;
          }
          ...
          "mRender": function ( data, type, full ) {
            return full.payment_datetime;
              ...
              {   "mData" : null, "sClass": "center",
          "mRender": function ( data, type, full ) {
          	return returnEmptyIfNull(full.vc_date);
          }
        },
        {   "mData" : null, "sClass": "right",
          "mRender": function ( data, type, full ) {
	...
	$(document).ready(function() {
	    ...
	    "aoColumnDefs": dataTableCustomColumnDef,
			"aoColumns": [
	            ...
	            { 	"mData" : null , "sClass": "center",
					"mRender": function ( data, type, full ) {
						return '<a href="#" onclick="openEditLangModal(this); return false;" class="btn btn-primary" rel="tooltip" data-title="Edit">Edit</a>';
					}
				}
	        ],
	        "aaSorting": [[ 1, "asc" ]],
...
$(document).ready(function() {
	if($("#inputChangeLang").size()>0){
		$("#inputChangeLang").change(function(){
			setSessionLang(this);
		});
	}
