...
function modernaChangeVcDate(elem){
	...
	if(data.order_lot_system.indexOf('moderna')!=-1){
		openModal("กรุณากดยืนยัน",'<b style="color:red; font-size:20px;">ท่านสามารถเลื่อนนัดได้ตลอดก่อนถึงวันฉีดจริง 24 ชม.</b><br/><br/><img src="https://cdn.jsdelivr.net/gh/pop-itsmartone/service-cdn@main/image/คำแนะนำ_moderna_2.jpg" style="max-width:100%" />',true,function(){
			hideModal();
		...
		bpkDataTable=$('#'+currentDatatableId).DataTable({
			...
			"aoColumns": [
	            ...
	            { 	"mData" : null, "sClass": "center",
					"mRender": function ( data, type, full ) {
						var html = '';
						if(full.receive_vaccine_place_id=='500' && full.vc_date_time_text=='' && full.vaccine_status_id=='W'
							&& ((parseInt(full.hos_queue_no)>=11652 && parseInt(full.hos_queue_no)<=25000) || $.inArray(full.hos_queue_no,['26314','26611'])!=-1)
