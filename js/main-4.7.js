...
function modernaChangeVcDate(elem){
	...
	if(data.order_lot_system.indexOf('moderna')!=-1){
		openModal("กรุณากดยืนยัน",'<b style="color:red; font-size:20px;">ท่านสามารถเลื่อนนัดได้ตลอดก่อนถึงวันฉีดจริง 48 ชม.</b><br/><br/><img src="https://cdn.jsdelivr.net/gh/pop-itsmartone/service-cdn@main/image/คำแนะนำ_moderna_รอบ1.jpg" style="max-width:100%" />',true,function(){
			hideModal();
...
function modernaChangeReceiver(elem){
	...
	if(data.order_lot_system.indexOf('moderna')!=-1){
		openModal("กรุณากดยืนยัน",'<b style="color:red; font-size:20px;">ท่านสามารถโอนสิทธิ์ได้เพียง 1 ครั้งเท่านั้น</b><br/><br/><img src="https://cdn.jsdelivr.net/gh/pop-itsmartone/service-cdn@main/image/คำแนะนำ_moderna_รอบ1.jpg" style="max-width:100%" />',true,function(){
			hideModal();
	            ...
	            { 	"mData" : null, "sClass": "center",
					"mRender": function ( data, type, full ) {
						...
						if(full.can_transfer=='T'){
							html+=full.status_text+'<br/><a href="#" onclick="modernaChangeReceiver(this);">กดโอนสิทธิ์</a>';
						}
						else{
	            ...
	            { 	"mData" : null, "sClass": "center",
					"mRender": function ( data, type, full ) {
						...
						if(full.can_change_date=='T'){
							html+=nl2br(full.vc_date_time_text)+'<br/><a href="#" onclick="modernaChangeVcDate(this);">กดเปลี่ยนวันเวลา</a>';
						}
						else{
  ...
  socket.on('receive_cid', function (msg) {
	...
	$(function(){
	            ...
	            { 	"mData" : null, "sClass": "center",
					"mRender": function ( data, type, full ) {
						if(full.vaccine_status_id=='D'){
							return '<span class="label label-success">'+full.status_text+'</span>';
						}

						var html = full.status_text;
				...
				},
        {
        	"mData" : null, "sClass": "center",
					"mRender": function ( data, type, full ) {
						if(full.receive_vaccine_place_id!=session_service_selected_branchcode){
							return '';
						}
						var returnHtml= '<div class="btn-group">'
		            		...
		            		if(full.vaccine_status_id=='D' && full.is_admin=='T'){
		            		...
		            		if(full.vaccine_status_id=='W' && full.dose=='1' && inJSONArray("CHANGE_VACCINE_APPT_PLACE",session_service_permission_include) != -1){
			            	...
			            	if(full.is_paid=='F' && isNullOrEmpty(full.company_name) && $.inArray(full.oc_product_id,['82','83'])!=-1 && inJSONArray("CASHIER",session_service_permission_include) != -1){
